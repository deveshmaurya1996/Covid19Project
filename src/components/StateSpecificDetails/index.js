...
class StateSpecificDetails extends Component {
  ...
  getStateSpecificData = async () => {
    ...
    const fetchedStateWiseData = await response.json()
    this.setState({
  ...
  renderStateSpecificData = () => (
    <ThemeContext.Consumer>
      {value => {
        ...
        const tableData = this.convertObjectsDataIntoListItemsUsingForInMethod()

        const singleState = tableData.filter(
        ...
        return (
          <div className={`state-specific-data-container ${backgroundClass}`}>
            ...
            <ul className="cases-types">
              {singleState.map(eachStateDetails => (
                <CaseCardItem
                  key={eachStateDetails.stateCode}
                  showConfirmed={showConfirmed}
                  showActive={showActive}
                  showRecovered={showRecovered}
                  showDeceased={showDeceased}
                  stateTotal={eachStateDetails}
            ...
            >
              {sortedArray.map(eachData => (
                <DistrictItem
                  key={eachData.districtName}
                  districtDetails={eachData}
                  showActiveCases={showActiveCases}
                  showDeceasedCases={showDeceasedCases}
                  showRecoveredCases={showRecoveredCases}
                  showConfirmedCases={showConfirmedCases}
